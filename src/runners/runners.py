...
def run_LLM(
    ...
    """
    Run single-stage LLM method for parameter generation.
    ...
    Args:
        ...
        effects: List of effect names to include in the prompt
)
    """
    ...
    config_initialization = Config(
            prompt=PromptFactory.LLM_PARAMETER_INITIALIZATION_PROMPT_DASP(
                fx_chain=fx_chain, instruction=instructionset.instruction,
                effects=effects,
            ),
...
def run_LLM_CLAP(
    ...
    """
    Run LLM+CLAP method using a single config: LLM initialization followed by
    guided semantic refinement.
    """
    parameter_engine = ParameterEngine()
    stage_info = {}
...
def run_LLM_LLM(
    ...
    refined_params_tensor, refined_params_dict, _, _, _ = parameter_engine.get_params(
...
def run_LLMLLM_vs_InstructFX2FX(
    methods: list[Method],
    instructionset_initialization: InstructionSet1,
    instructionset_refinement: InstructionSet1,
